...
class dqmc_evaluator : public alps::parapack::simple_evaluator {
public:
    dqmc_evaluator(alps::Parameters const&) {}
    virtual ~dqmc_evaluator() {}

    void evaluate(alps::ObservableSet& obs) const {
	std::cout << "Here I am doing nothing" << std::endl;
    }
};


PARAPACK_SET_VERSION("SU(2) Invariant Determinantal QMC Simulations for Ground State");
PARAPACK_REGISTER_EVALUATOR(alps::parapack::dqmc_evaluator, "Complex DQMC");
PARAPACK_REGISTER_ALGORITHM(cx_dqmc_worker, "Complex DQMC");
PARAPACK_REGISTER_PARALLEL_ALGORITHM(cx_dqmc_worker, "Complex DQMC");
